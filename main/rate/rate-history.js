...
(function(){
  var request, rateCache, moment, p, big, iserror, buildUrl, eachMinuteQuarter, roundMinuteQuarterTrade, roundMinuteQuarter, observers, notify, $off, $on, dateToTs, calcAvgTrade, calcAvg, loadRates, rateIndex, createRateIndex, getRate, getRateByPair, getOrLoadRate, out$ = typeof exports != 'undefined' && exports || this, toString$ = {}.toString;
  ...
  calcAvg = function(arg$){
    var ts, trades, accum, result, avg;
    ts = arg$[0], trades = arg$[1];
    accum = {
      sumOfRxA: new big(0),
      sumOfA: new big(0)
    };
    ...
    trades);
    avg = result.sumOfRxA.div(result.sumOfA).toString();
    return {
      ts: ts,
      avg: avg
    };
  };
  loadRates = curry$(function(arg$, cb){
    ...
    return request(currentUrl, function(err, response, body){
      ...
      loadRates({
        ...
        var allItems, this$ = this;
        if (err != null) {
          return cb("Error for " + url + "/" + nextTs + ": " + err);
        }
        allItems = p.map(calcAvg)(
        p.objToPairs(
        p.groupBy(roundMinuteQuarterTrade)(
        p.filter(function(it){
          return it.avg == null;
        })(
        p.uniqueBy(function(it){
          return it.globalTradeID;
        })(
        function(it){
          return it.concat(nextItems);
        }(
        items))))));
        cb(null, allItems);
      });
  ...
  out$.createRateIndex = createRateIndex = function(arg$, cb){
    ...
    return loadRates({
      ...
      var ri, ref$, ensureIndex;
      if (err != null) {
        return cbWrap(err);
      }
      ri = rateIndex[currencyPair] = (ref$ = rateIndex[currencyPair]) != null
        ? ref$
        : {};
      ensureIndex = function(arg$){
        var ts, avg;
        ts = arg$.ts, avg = arg$.avg;
        return ri[ts] = avg;
      };
      p.each(ensureIndex)(
      rates);
